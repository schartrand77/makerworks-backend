...
SCRIPT_PATH = os.path.join(
    PROJECT_ROOT, "extract_metadata.py"
)  # blender will call this
...
def main():
    if len(sys.argv) != 4:
        logger.error(
            "Usage: extract_metadata.py <model_path> <json_output> <preview_output>"
        )
        sys.exit(1)
    ...
    logger.info(
        f"Running Blender headless extraction for: {os.path.basename(model_path)}"
    )
